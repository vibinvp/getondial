...
import 'package:getondial/data/model/response/cart_suggested_item_model.dart';
import 'package:getondial/data/model/response/category_model.dart';
import 'package:getondial/data/model/response/item_model.dart';
import 'package:getondial/data/model/response/recommended_product_model.dart';
...
class StoreController extends GetxController implements GetxService {
  ...
  List<VideoDetailsList>? _videolist;
  ...
  List<VideoDetailsList>? get videoDetails => _videolist;
  ...
  Future<void> getVideoUrl(int moduleId) async {
    _videoModel = VideoModel();
    Response response = await storeRepo.getStoreVideoUrl(moduleId);
    if (response.statusCode == 200) {
      _videoModel = VideoModel.fromJson(response.body);
      _videolist = _videoModel?.value;
      update();
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<Store?> getStoreDetails(Store store, bool fromModule,
    ...
    if (store.name != null) {
      ...
      _store = null;
      Response response =
          await storeRepo.getStoreDetails(store.id.toString(), fromCart);
      if (response.statusCode == 200) {
