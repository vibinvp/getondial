...
import 'package:getondial/data/api/api_checker.dart';
import 'package:getondial/data/api/api_client.dart';
import 'package:getondial/data/model/body/place_order_body.dart';
import 'package:getondial/data/model/response/distance_model.dart';
import 'package:getondial/data/model/response/order_cancellation_body.dart';
...
import 'package:getondial/data/model/response/refund_model.dart';
import 'package:getondial/data/model/response/response_model.dart';
import 'package:getondial/data/model/response/store_model.dart';
import 'package:getondial/data/model/response/timeslote_model.dart';
...
import 'package:getondial/helper/network_info.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  Future<double?> getExtraCharge(double? distance) async {
    print('getExtraCharge ---- caalleddd');
    try {
      _extraCharge = null;
      ...
      return _extraCharge!.roundToDouble();
    } catch (e) {
      print(e.toString());
    }
    return null;
  ...
  Future<void> getRunningOrders(int offset, {bool isUpdate = false}) async {
    ...
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        // print("onMessage running orders ${_runningOrderModel?.orders?[0]}");
        //  print("onMessage running order model ${_runningOrderModel?.totalSize}");
        //   print("onMessage running order model ${_runningOrderModel?.limit}");
      } else {
  ...
  Future<void> getHistoryOrders(int offset, {bool isUpdate = false}) async {
    ...
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        //  print("onMessage historyOrder order ${_historyOrderModel?.orders}");
        //  print("onMessage historyOrder offset  ${_historyOrderModel?.offset}");
        //  print("onMessage historyOrder limit  ${_historyOrderModel?.totalSize}");
      } else {
  ...
  void setTypeIndex(int dmType, bool notify) {
    _typeIndex = dmType;
    if (notify) {
      update();
    }
  ...
  Future<void> placeOrder(
    ...
    print(
        'controller amount------------------>>   $amount----------order bodyy --------->>${placeOrderBody.orderAmount}');
    _isLoading = true;
    update();
    Response response =
  ...
  Future<double?> getDistanceInKM(LatLng originLatLng, LatLng destinationLatLng,
    ...
    if (!fromDashboard) {
      await getExtraCharge(_distance!.roundToDouble());
    }
    print('returnnnn _distance ----->>    $_distance  ');
    update();
    return _distance;
  }
